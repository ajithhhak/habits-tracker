...
export async function sendOTPEmail(to, otp, name = '') {
    ...
    <body style="margin:0;padding:0;background:#f0fdf9;font-family:Inter,sans-serif">
      <div style="max-width:480px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        <div style="background:linear-gradient(135deg,#0d9488,#0f766e);padding:32px;text-align:center">
          <h1 style="color:#fff;margin:0;font-size:24px;font-weight:800">HabitSync ✦</h1>
        ...
        <div style="background:#f9fafb;padding:16px;text-align:center">
          <p style="color:#d1d5db;font-size:11px;margin:0">© 2025 HabitSync. Built with ♥</p>
        </div>
      </div>
    </body>
    </html>
  ...
  await transporter.sendMail({
    ...
    to,
    subject: `${otp} — Your HabitSync verification code`,
    html,
  })
}
...
export async function sendWelcomeEmail(to, name) {
    ...
    <body style="margin:0;padding:0;background:#f0fdf9;font-family:Inter,sans-serif">
      <div style="max-width:480px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        <div style="background:linear-gradient(135deg,#0d9488,#0f766e);padding:32px;text-align:center">
          ...
          <h1 style="color:#fff;margin:8px 0 0;font-size:22px">Welcome to HabitSync!</h1>
        </div>
        <div style="padding:32px">
  ...
  await transporter.sendMail({
    from: process.env.EMAIL_FROM,
    to,
    subject: `Welcome to HabitSync, ${name}! 🌱`,
    html,
  })
}
