      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="card glass overflow-hidden !p-0">
          <div className="overflow-auto custom-scrollbar">
            ...
            {/* Summary strip */}
            <div className="bg-surface-100 dark:bg-surface-800 text-surface-900 dark:text-surface-50 px-4 py-3 border-b border-surface-200 dark:border-surface-700">
              <span className="text-sm font-bold tracking-wider uppercase flex items-center gap-2"><CheckSquare size={16} className="text-brand-500" /> Daily Goals</span>
            </div>

            {/* Completed / Not / % rows */}
            {[
              { label: 'Completed',     bgClass: 'bg-brand-50 dark:bg-brand-900/20', fgClass: 'text-brand-700 dark:text-brand-300', val: d => completedByDay[d-1] },
              { label: 'Not Completed', bgClass: 'bg-red-50 dark:bg-red-900/20', fgClass: 'text-red-700 dark:text-red-300', val: d => habits.length - completedByDay[d-1] },
              { label: '% Completed',   bgClass: 'bg-amber-50 dark:bg-amber-900/20', fgClass: 'text-amber-700 dark:text-amber-300', val: d => pctByDay[d-1] + '%' },
            ].map(row => (
              <div key={row.label} className="flex border-b border-surface-200 dark:border-surface-700">
                <div className={`w-48 flex-shrink-0 flex items-center justify-end px-3 py-1.5 text-xs font-bold ${row.bgClass} ${row.fgClass}`}>
                  {row.label}
                </div>
                {days.map(d => (
                  <div key={d} className={`flex-1 text-center py-1.5 text-xs font-bold border-r border-surface-200/50 dark:border-surface-700/50 ${row.bgClass} ${row.fgClass}`}
                       style={{ minWidth: 36 }}>
                    {row.val(d)}
                  </div>
                ))}
            ...
            <div className="flex border-b border-surface-200 dark:border-surface-700">
              <div className="w-48 flex-shrink-0 bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300 text-xs font-bold px-3 py-2 flex items-center">Goal</div>
              {weekGroups.map((wg, wi) => {
                const span  = wg.end - wg.start + 1
                return (
                  <div key={wi} className="flex items-center justify-center py-2 text-xs font-bold tracking-wide border-r border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 text-surface-600 dark:text-surface-400"
                       style={{ flex: span }}>
                    Week {wi + 1}
                  </div>
                )
            ...
            {/* Day name row */}
            <div className="flex border-b border-surface-200 dark:border-surface-700">
              <div className="w-48 flex-shrink-0 bg-surface-50 dark:bg-surface-800/50" />
              {days.map(d => {
                const dow = (firstDow + d - 1) % 7
                return (
                  <div key={d} className="flex-1 text-center py-1.5 text-xs font-semibold border-r border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 text-surface-500 dark:text-surface-400"
                       style={{ minWidth: 36 }}>
                    {DOW[dow]}
                  </div>
                )
            ...
            <div className="flex border-b border-surface-200 dark:border-surface-700">
              ...
              {days.map(d => {
                ...
                return (
                  <div key={d} className="flex-1 text-center py-2 text-sm font-bold border-r border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 text-surface-800 dark:text-surface-200"
                       style={{ minWidth: 36 }}>
                    <span className={isToday ? 'inline-flex items-center justify-center w-6 h-6 rounded-full bg-brand-500 text-white shadow-md' : ''}>
                      {d}
                    </span>
                  </div>
                )
            ...
            {/* Habit rows */}
            {habits.map((habit, hi) => {
              const bgClass = hi % 2 === 0 ? 'bg-surface-50 dark:bg-surface-800/30' : 'bg-white dark:bg-surface-900'
              return (
                <div key={habit._id} className="flex border-b border-surface-200 dark:border-surface-800 hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors group">
                  <div className={`w-48 flex-shrink-0 flex items-center gap-3 px-3 py-2 border-r border-surface-200 dark:border-surface-800 text-sm font-medium text-surface-800 dark:text-surface-200 relative ${bgClass}`}>
                    <span className="text-lg">{habit.icon}</span>
                    <span className="truncate flex-1 pr-6">{habit.name}</span>
                    <button onClick={() => setEditingHabit(habit)} className="opacity-0 group-hover:opacity-100 transition-all p-1.5 hover:bg-white dark:hover:bg-surface-700 rounded-lg absolute right-2 shadow-sm border border-transparent hover:border-surface-200 dark:hover:border-surface-600 bg-surface-100 dark:bg-surface-800 text-surface-500 dark:text-surface-400">
                      <Settings size={14} />
                    </button>
                  </div>
                  {days.map(d => {
                    ...
                    return (
                      <div key={d} onClick={() => toggleTick(habit._id, d)}
                           className={`flex-1 flex items-center justify-center border-r border-surface-200 dark:border-surface-800 cursor-pointer hover:opacity-80 transition-all ${checked ? 'bg-brand-500/10 dark:bg-brand-500/20' : bgClass}`}
                           style={{ minWidth: 36, height: 44 }}>
                        {isSaving
                          ? <Loader2 size={16} className="animate-spin text-brand-500" />
                          : checked
                          ? <div className="w-5 h-5 rounded-md bg-brand-500 text-white flex items-center justify-center shadow-sm">
                              <CheckSquare size={14} />
                            </div>
                          : <div className="w-5 h-5 rounded-md border-2 border-surface-300 dark:border-surface-600 group-hover:border-brand-300 dark:group-hover:border-brand-700 transition-colors" />
                        }
                      </div>
                    )
                  })}
            ...
            <div className="flex border-b border-surface-200 dark:border-surface-800">
              <div className="w-48 flex-shrink-0 flex items-center gap-2 px-3 py-3 border-r border-surface-200 dark:border-surface-800 bg-surface-50/50 dark:bg-surface-800/20">
                <input value={newHabit} onChange={e => setNewHabit(e.target.value)}
                       onKeyDown={e => e.key === 'Enter' && addHabit()}
                       placeholder="+ Add a habit…"
                       className="text-sm text-surface-600 dark:text-surface-300 bg-transparent outline-none w-full placeholder-surface-400 dark:placeholder-surface-500" />
                {newHabit && (
                  <button onClick={addHabit} disabled={addingHabit}
                          className="text-xs text-brand-600 dark:text-brand-400 font-bold flex-shrink-0 px-2 py-1 bg-brand-50 dark:bg-brand-900/30 rounded-md hover:bg-brand-100 dark:hover:bg-brand-900/50 transition-colors">Add</button>
                )}
              </div>
              {days.map(d => <div key={d} className="flex-1 border-r border-surface-200 dark:border-surface-800 bg-surface-50/50 dark:bg-surface-800/20" style={{ minWidth: 36, height: 48 }} />)}
            </div>

            {/* Mood row */}
            <div className="bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/10 dark:to-purple-900/10">
              <div className="flex">
                <div className="w-48 flex-shrink-0 flex items-center px-3 py-4 border-r border-pink-100 dark:border-pink-900/30 text-sm font-bold text-pink-700 dark:text-pink-400">
                  <span className="mr-2">✨</span> Daily Mood
                </div>
                {days.map(d => {
                  const dateKey = `${monthKey}-${String(d).padStart(2,'0')}`
                  const mood = logs[dateKey]?.mood || ''
                  return (
                    <div key={d} className="flex-1 relative flex items-center justify-center border-r border-pink-100 dark:border-pink-900/30 cursor-pointer hover:bg-pink-100/50 dark:hover:bg-pink-900/20 transition-colors"
                         style={{ minWidth: 36, height: 52 }}
                         onClick={() => setMoodPicker(moodPicker === d ? null : d)}>
                      <span className="text-xl leading-none">{mood || <span className="text-surface-300 dark:text-surface-600 text-xs">·</span>}</span>
                      {moodPicker === d && (
                        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-50
                                        bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl shadow-2xl p-2 flex gap-1 whitespace-nowrap animate-fade-in"
